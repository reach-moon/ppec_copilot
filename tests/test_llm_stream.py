...
def test_llm_stream_endpoint():
    """Test the new llm-stream endpoint"""
...
def test_chat_completions_routing_to_llm():
    """Test that chat-completions properly routes to llm-stream for non-ragflow models"""
    
    # Mock the LLM response
    mock_llm = MagicMock()
    mock_chunk1 = MagicMock()
    mock_chunk1.content = "Hello from LLM!"
    ...
    with patch('app.api.endpoints.v1.chat.get_llm', return_value=mock_llm):
        ...
        assert chunk.object == "chat.completion.chunk"
        assert chunk.choices[0].delta.content == "Hello from LLM!"
        assert chunk.choices[0].delta.role == "assistant"
        assert chunk.model == "qwen-test"
