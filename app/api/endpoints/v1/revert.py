...
async def revert_conversation(request: RevertRequest):
    ...
    - turn_id: 定位到该会话需要回滚到的“时间点”（即哪一次交互）。
    """
    logger.info(f"接收到回滚请求，Session: {request.session_id}, 回滚至 Turn: {request.turn_id}")
    try:
        # 调用我们重构后的 revert_to_turn 方法
        # 这个方法会删除 Mem0 中所有在目标 turn_id 之后存储的记忆
